...
public:
    ...
    const handleTestComplete = (results) => {
        ...
        if (results.allPassed) {
            ...
            if (typeof window !== "undefined") {
                alert(
                    "🎉 Congratulations! All tests passed! You have successfully solved this problem!"
                );
            }
        }
    };
    ...
    const handleRunCode = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                const executionResult = await waitForSubmissionCompletion(
                    result.data.token
                );
    ...
    const handleSubmitCode = async () => {
        ...
        try {
            ...
            const response = await fetch(
                `${import.meta.env.VITE_API_URL}/submissions/test-cases`,
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                ...
                for (const token of result.data.tokens) {
                    try {
                        const executionResult =
                            await waitForSubmissionCompletion(token);
                        testCaseResults.push(executionResult);
                    } catch (error) {
                        console.error(
                            `Error getting result for token ${token}:`,
                            error
                        );
                    ...
                    <div className={styles.problemContent}>
                        ...
                        {activeTab === "solutions" && (
                            <div className={styles.solutions}>
                                <h3>Solutions</h3>
                                <p>
                                    Community solutions will be available here.
                                </p>
                            </div>
                        )}
                        {activeTab === "submissions" && (
                            <div className={styles.submissions}>
                                <h3>Submissions</h3>
                                <p>
                                    Your submission history will be available
                                    here.
                                </p>
                            </div>
                        )}
                        ...
                        <div className={styles.actions}>
                            ...
                            <button
                                className={`${styles.actionBtn} ${
                                    styles.submitBtn
                                } ${allTestsPassed ? styles.success : ""}`}
                                onClick={handleSubmitCode}
                                disabled={
                                    isRunning || isSubmitting || !allTestsPassed
                                }
                                title={
                                    !allTestsPassed
                                        ? "Complete all test cases first"
